    ...
    def exe(self, query):
        try:
            with self.conn.begin() as t:
                ...
                assert (
                    result.rowcount <= 1
                ), f"{Style.BRIGHT}{Fore.RED}{Back.BLACK}<S.C.A.R.S> ERROR{Style.RESET_ALL} : Multiple rows returned for single row query"
...
def check_Sdb_tables():
    for table_name in tables:
        if bot_inspec.has_table(table_name):
            print("---------------------------------------------------------------")
            print(
                f"<S.C.A.R.S> {Style.BRIGHT}{Fore.YELLOW}{Back.BLACK}CHECKING{Style.RESET_ALL} : SDB Table - {table_name}... {Style.BRIGHT}{Fore.GREEN}{Back.BLACK}[OK]{Style.RESET_ALL}"
            )
        else:
            print("---------------------------------------------------------------")
            print(
                f"<S.C.A.R.S> {Style.BRIGHT}{Fore.YELLOW}{Back.BLACK}CHECKING{Style.RESET_ALL} : SDB Table - {table_name}... {Style.BRIGHT}{Fore.YELLOW}{Back.BLACK}[NOT FOUND]{Style.RESET_ALL}"
            )
            print(
                f"<S.C.A.R.S> {Style.BRIGHT}{Fore.YELLOW}{Back.BLACK}CONSTRUCTING{Style.RESET_ALL} :  SDB Table {table_name}..."
            )
            table = botdb_metadata.tables[table_name]
            table.create(botdb.engine)
            print(
                f"<S.C.A.R.S> TABLE CONSTRUCTION : {Style.BRIGHT}{Fore.GREEN}{Back.BLACK}[OK]{Style.RESET_ALL}"
            )
...
def insert_Table_data(table_name, table):
    if f"{table_name}_data" in data_inserts:
        print(
            f"<S.C.A.R.S> {Style.BRIGHT}{Fore.YELLOW}{Back.BLACK}CONSTRUCTING{Style.RESET_ALL} : SDB {table_name} Data..."
        )
...
def get_Channels_list(client):
    print(
        f"<S.C.A.R.S> {Style.BRIGHT}{Fore.YELLOW}{Back.BLACK}CHECKING{Style.RESET_ALL} : Active Channel List..."
    )
    for i in range(2):
        for channel in getall_Channels():
            try:
                ...
                if channel_obj:
                    ...
                    print(
                        f"<S.C.A.R.S> Channel {channel['name']} {Style.BRIGHT}{Fore.YELLOW}{Back.BLACK}[INACTIVE]{Style.RESET_ALL}"
                    )
            except Exception as err:
                traceback.print_exc()
                logs.log_Event(f"Error in Active Channel List - OBJECT ERROR: ", err)
                logs.log_Event(
                    f"Error in Active Channel List - STACKTRACE: ",
...
def get_Channel_byname(name: str):
    result = botdb.exe(
        select(channels_tbl.c.id, channels_tbl.c.name, channels_tbl.c.type).where(
            channels_tbl.c.name == name
        )
    )
    return result
...
def update_Channel(channel):
    botdb.exe_Commit(
        channels_tbl.update()
        .where(channels_tbl.c.id == channel.id)
        .values(id=channel.id, name=channel.name, type=channel.type)
    )


def save_Channel(channel):
    botdb.exe_Commit(
        channels_tbl.insert().values(
...
def check_Superuser_by_did(discord_id: int):
    result = botdb.exe_Bool(
        select(survivors_tbl.c.username)
        .where(survivors_tbl.c.discord_id == discord_id)
        .where(survivors_tbl.c.super_user.isnot(None))
    )
    return result
...
def getall_Superusers():
    result = botdb.exe_Multi(
        survivors_tbl.select().where(survivors_tbl.c.super_user.isnot(None))
    )
    return result
...
def update_Surv(survivor):
    botdb.exe_Commit(
        survivors_tbl.update()
        .where(survivors_tbl.c.discord_id == survivor.discord_id)
        .values(
            username=survivor.username,
            display_name=survivor.display_name,
            pz=survivor.pz,
...
def remove_Surv_by_did(discord_id: int):
    botdb.exe_Commit(
        survivors_tbl.delete().where(survivors_tbl.c.discord_id == discord_id)
    )
...
def get_Response_by_title(title: str):
    result = botdb.exe(
        select(
            responses_tbl.c.title.responses_tbl.c.section, responses_tbl.c.body
        ).where(responses_tbl.c.title == title)
    )
    return result
...
def get_Random_response_by_sec(section: str):
    result = botdb.exe(
        select(responses_tbl.c.title, responses_tbl.c.section, responses_tbl.c.body)
        .where(responses_tbl.c.section == section)
...
def update_SYS_controls(controls):
    botdb.exe_Commit(
        controls_tbl.update()
        .where(controls_tbl.c.name == controls.name)
        .values(
            bool=controls.bool,
            dt=controls.dt,
            int=controls.int,
...
def get_All_pz_servers():
    result = botdb.exe_Multi(
        select(
            servers_tbl.c.id,
            servers_tbl.c.game,
            servers_tbl.c.iwbums_server,
            servers_tbl.c.test_server,
            servers_tbl.c.name,
            servers_tbl.c.ip,
            servers_tbl.c.port,
            servers_tbl.c.active,
            servers_tbl.c.wiped,
            servers_tbl.c.passwd,
            servers_tbl.c.adminpw,
            servers_tbl.c.apiport,
...
def update_pz_server(server):
    botdb.exe_Commit(
        servers_tbl.update()
        .where(servers_tbl.c.id == server.id)
        .values(
            game=server.game,
...
def get_LL_template(game: str, event_type: str, level: str):
    result = botdb.exe(
        select(ll_templates_tbl.c.title, ll_templates_tbl.c.body)
        .where(
            and_(
                ll_templates_tbl.c.game == game,
                ll_templates_tbl.c.event_type == event_type,
                ll_templates_tbl.c.level == level,
            )
        )
        .order_by(sa.func.rand())
        .limit(1)
...
def delete_LL_message(ll_message):
    botdb.exe_Commit(
        ll_messages_tbl.delete().where(ll_messages_tbl.c.id == ll_message.id)
    )
...
def get_Pzchar(server_id, username):
    result = pzdb.exe_Lite(
        select(player.c.server_id, player.c.username).where(
            and_(
...
def get_Pzcharname(server_id, username):
    result = pzdb.exe_Lite(
        select(player.c.fullname.label("charname"))
        .where(and_(player.c.server_id == server_id, player.c.username == username))
...
def check_Pzchar_exists(server_id, username):
    result = pzdb.exe_Bool(
        select(player.c.username).where(
            and_(
                player.c.username == username,
                player.c.died_ingame.is_(None),
                player.c.server_id == server_id,
...
def get_LL_pz_data_by_did(discord_id: int, table):
    ...
    result = botdb.exe(
        select(
            ...
            selected_table.c.zbits,
        ).where(selected_table.c.discord_id == discord_id)
    )
    return result
...
def get_LL_pz_data_by_server(server_id: int, table):
    ...
    result = botdb.exe_Multi(
        select(
            ...
            selected_table.c.kdr,
            selected_table.c.zbits,
        ).where(selected_table.c.server_id == server_id)
    )
    return result
...
def save_LL_pz_data_challengecopy(ll_pz_data_challengecopy):
    botdb.exe_Commit(
        insert(ll_pz_data_challengecopy_tbl).values(
...
def delete_LL_pz_challengecopy_by_id(challenge_id: int):
    botdb.exe_Commit(
        ll_pz_data_challengecopy_tbl.delete().where(
            ll_pz_data_challengecopy_tbl.c.id == challenge_id
        )
    )
...
def get_Pz_kills(server_id, username):
    result = pzdb.exe_Lite(
        select(player.c.vp.label("kills")).where(
            and_(
                player.c.server_id == server_id,
                player.c.username == username,
                player.c.died_ingame.is_(None),
...
def get_Pz_total_kills(server_id, username):
    result = pzdb.exe_Lite(
        select(sa.func.sum(player.c.vp).label("kills")).where(
            and_(player.c.server_id == server_id, player.c.username == username)
        )
    )
    return result
...
def get_Pz_skill(server_id, username, skill):
    result = pzdb.exe_Lite(
        select(player_xp.c.current.label(skill))
        .select_from(
...
def get_Pz_deaths(server_id, username):
    result = pzdb.exe_Lite_multi(
        select(player.c.died_ingame.label("deaths")).where(
            and_(
                player.c.server_id == server_id,
                player.c.username == username,
...
def get_Pz_last_death(server_id, username):
    result = pzdb.exe_Lite(
        select(player.c.died_ingame.label("last_death"))
...
def get_Pz_last_death_rt(server_id, username):
    result = pzdb.exe_Lite(
        select(player.c.died_time.label("last_death_rt"))
        .where(player.c.server_id == server_id)
        .where(player.c.username == username)
        .where(player.c.died_ingame.isnot(None))
        .order_by(player.c.died_ingame.desc())
...
def save_Pz_challenge(self):
    botdb.exe_Commit(
        ll_pz_challenges_tbl.insert().values(
            name=self.name,
...
def delete_Pz_challenge(self):
    botdb.exe_Commit(
        ll_pz_challenges_tbl.delete().where(ll_pz_challenges_tbl.c.id == self.id)
    )
...
def update_sdrop_order(self):
    botdb.exe_Commit(
        ll_pz_sdrop_orders_tbl.update()
        .where(ll_pz_sdrop_orders_tbl.c.id == self.id)
        .values(
            discord_id=self.discord_id,
            charname=self.charname,
            cost=self.cost,
...
def delete_sdrop_order(self):
    botdb.exe_Commit(
        ll_pz_sdrop_orders_tbl.delete().where(ll_pz_sdrop_orders_tbl.c.id == self.id)
    )
...
def update_sdrop_type(self):
    botdb.exe_Commit(
        ll_pz_sdrop_tbl.update()
        .where(ll_pz_sdrop_tbl.c.name == self.name)
        .values(
            original_cost=self.original_cost,
            variation=self.variation,
            current_cost=self.current_cost,
